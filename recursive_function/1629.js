...
1.  **분할 정복 (Divide and Conquer)**
    -   이 문제의 핵심 알고리즘 패러다임입니다. A^B를 구하기 위해 B를 계속 절반으로 나누어 문제를 더 작은 단위로 쪼개고(Divide), 작은 문제의 답을 조합하여 원래 문제의 답을 구합니다(Conquer).
...
3.  **모듈러 산술 (Modular Arithmetic)**
    ...
    -   이 성질 덕분에 거듭제곱을 하는 중간 과정마다 `% C` 연산을 적용하여, 수가 무한정 커지는 것을 막고 자료형의 범위를 벗어나지 않게 할 수 있습니다. 코드의 모든 곱셈 뒤에 `% C`가 붙는 이유입니다.
